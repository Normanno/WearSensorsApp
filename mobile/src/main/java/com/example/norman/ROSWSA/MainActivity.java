        ...
        toggleROS.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if(b){
                    ...
                    tb.setEnabled(false);
                    AsyncTask checker = new AsyncTask() {
                        @Override
                        protected Object doInBackground(Object[] objects) {
                            URL url;
                            String ip_address = deviceSensingManager.getRosHostname();
                            String path = deviceSensingManager.getRosPath();
                            Integer port = deviceSensingManager.getRosPort();

                            path = (path.charAt(0) == '/' ? path:"/"+path);

                            Log.d(debugTag,"URL : http://"+ip_address+":"+port+path);

                            try {
                                if( ip_address == null || ip_address.isEmpty())
                                    throw new ROSEmptyURL();
                                if(port != 0)
                                    url = new URL("http://"+ip_address+":"+port+path);
                                else
                                    url = new URL("http://"+ip_address+path);
                                Log.d(debugTag,"URL : "+url.toString());
                                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                                Log.d(debugTag, "connected");
                                conn.setRequestMethod("HEAD");
                                Log.d(debugTag, "*** Response code "+conn.getResponseCode());
                                if( conn.getResponseCode() == 200) {
                                    rosDataSender.startSending(url);
                                }
                                ...
                                errorSnack.show();
                                Log.d(debugTag, "*** EXCEPTION: "+ee.toString());
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        toggleROS.toggle();
